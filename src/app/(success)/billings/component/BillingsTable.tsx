            ...
            <div className='w-full'>
                <div className='flex justify-between items-center py-4 px-10 bg-white'>
                    <div className='flex items-center gap-2'>
                        <span className='text-xs'>Todas las facturas</span>
                    </div>
                    <button onClick={() => router.push("/billings/new")} className="text-black rounded-md flex items-center text-xs border-slatesecondary shadow-md  focus:ring-4 focus:ring-blue-300 font-medium  px-3 py-1  w-auto">
                ...
                <div className="relative overflow-x-auto">
                    {currentItems.length > 0 ? <div className="w-full text-sm text-left rtl:text-right text-gray-500  ">
                        ...
                        <div className=''>
                            {
                                currentItems?.map(el => (<div key={el.id} className="bg-white grid grid-cols-3 md:grid-cols-4 lg:grid-cols-7 border-b py-6 mb-4 rounded-md hover:cursor-pointer hover:bg-slate-100 ">
                                    <div className="md:px-6 py-4 text-xs font-medium  my-auto">
                                        <div className='flex flex-col '>
                                            <Link className='px-1 text-xs py-1 text-blueprimary hover:underline' href={`/billings/${el.id}`}>
                                                {"#"}{el.work.id} {el.work.name}
                                            </Link>
                                            <p>{el.client.company}</p>
                                        </div>
                                    </div>
